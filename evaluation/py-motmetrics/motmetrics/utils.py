...
def compute_global_aligment_score(
    allframeids, fid_to_fgt, fid_to_fdt, num_gt_id, num_det_id, dist_func, iou_class_penalties = None
):
    ...
    for fid in allframeids:
        ...
        if len(oids) > 0 and len(hids) > 0:
            ...
            similarity = dist_func(fgt.values, fdt.values, return_dist=False, iou_class_penalty = iou_class_penalties[str(fid)])
...
def compare_to_groundtruth_reweighting(gt, dt, dist="iou", distfields=None, distth=(0.5), iou_class_penalties = None):
    ...
    def compute_iou(a, b, return_dist, iou_class_penalty = None):
        return iou_matrix(a, b, max_iou=distth, return_dist=return_dist, iou_class_penalty=iou_class_penalty)
    ...
    global_alignment_score = compute_global_aligment_score(
        allframeids, fid_to_fgt, fid_to_fdt, num_gt_id, num_det_id, compute_dist, iou_class_penalties = iou_class_penalties
    )
    ...
    for fid in allframeids:
        ...
        if len(oids) > 0 and len(hids) > 0:
            gt_ids = np.array(oids.values) - 1
            dt_ids = np.array(hids.values) - 1
            dists = compute_dist(fgt.values, fdt.values, return_dist=False, iou_class_penalty=iou_class_penalties[str(fid)])
            weighted_dists = (
