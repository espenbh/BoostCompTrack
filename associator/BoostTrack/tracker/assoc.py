############ Changed #############
'''
 - Change from lap to scipy for linear assignment due to version incompatibility
'''
#################################
...
def match(cost_matrix: np.ndarray, threshold: float) -> np.ndarray:
    if cost_matrix.size > 0:
        ...
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            ...
            matched_indices = np.vstack([x, y]).T
    else:
...
def linear_assignment(detections: np.ndarray, trackers: np.ndarray,
    ...
    for m in matched_indices:
        valid_match = iou_matrix[m[0], m[1]] >= threshold  or (False if emb_cost is None else (iou_matrix[m[0], m[1]] >= threshold / 2 and emb_cost[m[0], m[1]] >= 0.75)) #or (len(turns) > 0 and turns[m[1]] > 0 and iou_matrix[m[0], m[1]] > 0.001)
        if valid_match:
...
def associate(
    ...
    if detection_confidence is not None and track_confidence is not None:
        ...
        #if len(turns) > 0:
            #conf[iou_matrix < 0.05] = 0
            #conf[iou_matrix < iou_threshold] = conf[iou_matrix < iou_threshold]/4
        ###############
    ...
    #la_results = linear_assignment(detections, trackers, iou_matrix, cost_matrix, iou_threshold, turns, emb_cost)
    matches, unmatched_detections, unmatched_trackers, cost_matrix = linear_assignment(detections, trackers, iou_matrix, cost_matrix, iou_threshold, emb_cost)

    idces = np.where(turns)[0]
    if idces.shape[0] > 0:
        for t in idces:
            if t in unmatched_trackers:
                best_det = np.argmax(iou_matrix[:,t])
                if best_det in unmatched_detections and iou_matrix[best_det, t] > 0.05:
                    matches = np.vstack([matches, np.array([best_det, t])])
        
    
    #return [i for i in la_results[:-1]] + [iou_matrix]
    return [matches, unmatched_detections, unmatched_trackers, iou_matrix]